...
Històricament, la comprensió de la figura humana va començar amb la detecció d'objectes tradicional (\textit{Object Detection}), on els sistemes es limitaven a emmarcar l'individu dins d'una capsa delimitadora (\textit{bounding box}). Com a resposta a la manca d'informació biomecànica d'aquest enfocament, la recerca es va centrar en els mètodes basats en parts (\textit{Part-based models})\cite{partimagenet_2024}, que identificaven extremitats de manera independent per després intentar ajuntar-les.

L'autèntica revolució ha arribat amb les xarxes neuronals profundes, que permeten localitzar directament les coordenades espacials \textit{(x, y)} de cada articulació \cite{zheng_hpe_survey}. L'arquitectura exacta d'aquesta sortida depèn de la topologia anatòmica utilitzada: d'una banda, el format \textit{COCO} defineix un esquelet bàsic de 17 \textit{keypoints}, mentre que el format \textit{BlazePose} l'expandeix fins als 33 \textit{keypoints}, capturant amb detall les mans i els peus (vegeu la Figura \ref{fig:keypoint_mapping}).
...
Aquesta mètrica permet calcular el \textit{mean Average Precision} (\textit{mAP}), que és el valor que comparem en les auditories de models d'aquest projecte.

\subsection{Fonaments del Suavitzat Temporal: Filtre Savitzky-Golay}
En l'extracció de poses \textit{frame} a \textit{frame}, el soroll de la predicció pot generar vibracions artificials (\textit{jitter}). Per resoldre-ho, s'utilitzen filtres de suavitzat temporal, sent el filtre de \textit{Savitzky-Golay} \cite{savgol} un dels més utilitzats en el processament de senyal biomecànic.
...
Aquesta operació aconsegueix suavitzar la trajectòria de les articulacions sense eliminar els moviments reals d'alta velocitat de la gimnasta.
...
Dins d'aquestes, les xarxes \textit{Long Short-Term Memory} (\textit{LSTM}) \cite{lstm_rnn_2021} són les més eficaces perquè eviten que la informació es perdi en seqüències llargues. Com es mostra a la Figura \ref{fig:lstm}, un bloc LSTM controla el flux d'informació mitjançant tres portes:
...
Un cop seleccionat el model \textit{Sapiens-2B} com a motor principal d'extracció, s'ha implementat el mòdul de processament de senyal per mitigar el soroll de l'estimació espacial. L'aplicació pràctica del filtre de \textit{Savitzky-Golay} va generar un debat tècnic important durant les primeres fases del projecte.

A causa dels primers assajos obtinguts en els girs de perfil (vegeu la Figura \ref{fig:compare_bad}), es va valorar la possibilitat de suprimir-lo de l'arquitectura. En moviments on la gimnasta experimenta una alta freqüència de rotació o auto-oclusions severes, l'ajust del polinomi local tendia a difuminar l'intercanvi ràpid de cames, provocant un efecte de sobre-suavitzat (\textit{oversmoothing}) que deformava la morfologia de l'esquelet real.
...
No obstant això, aquesta decisió es va revertir en comprovar la seva importància crítica per a l'estabilitat global del \textit{pipeline}. Durant les extraccions massives de vídeos per al \textit{dataset}, s'ha demostrat que el filtre és una eina fonamental per contrarestar el \textit{frame dropping} (instants on la confiança de la xarxa neuronal descendeix dràsticament i no es detecta cap esquelet). En seqüències d'acrobàcies d'extensió fluida, l'ajust polinòmic assoleix un alineament biomecànic quasi perfecte (vegeu la Figura \ref{fig:compare_good}), garantint un senyal continu i net per al motor de regles posterior.
...
Aquestes seqüències estructurades s'han estandarditzat a una longitud de 30 fotogrames per bloc i s'han utilitzat per entrenar el classificador \textit{LSTM} descrit a l'estat de l'art. A nivell d'implementació amb \textit{PyTorch}, el model s'ha configurat amb un vector d'entrada adaptat directament a les coordenades bidimensionals dels 33 \textit{keypoints} de la topologia normalitzada. Després de processar el bloc seqüencial temporal, la xarxa transmet el darrer estat ocult a una capa lineal de sortida amb activació \textit{Softmax}, la qual s'encarrega d'assignar una probabilitat de pertinença a cadascuna de les quatre classes acrobàtiques.
...
L'anàlisi comparativa justifica l'elecció final de \textit{Sapiens-2B}. Mentre que \textit{YOLO26x-pose} lidera el banc de proves genèric, pateix una degradació severa al domini gimnàstic (el seu \textit{mAP@0.5} cau fins al 19.2\%). Això demostra que les oclusions i inversions penalitzen greument les arquitectures \textit{Top-Down}. En contraposició, l'enfocament de \textit{Vision Transformers} de \textit{Sapiens-2B} demostra una comprensió estructural global superior, retenint un \textit{PCK@0.2} del 72.7\% que garanteix la continuïtat de l'esquelet.
...
Per contra, la Figura \ref{fig:result_bad} il·lustra un cas de fallada estructural causat per les limitacions temporals discutides. En aquest escenari, la xarxa \textit{LSTM} genera un fals positiu en classificar un fragment de transició (l'aterratge de la gimnasta) com una classe acrobàtica activa. 
