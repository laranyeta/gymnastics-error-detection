...
\textbf{OpenCV} & Motor de visió encarregat del tractament de fotogrames, el dibuix vectorial de l'esquelet i el mapeig de coordenades 3D a píxels 2D. \\
...
A la Taula \ref{tab:planificacio} es desglossa la temporalitat d'aquestes fases. Com s'observa, el nivell de seguiment de la planificació és òptim, permetent dedicar la recta final del semestre al poliment de la interfície d'usuari i a la validació experimental exhaustiva.
...
Aquesta secció detalla l'execució tècnica del projecte, estructurada en el pipeline complet de processament: des de l'auditoria de models fins al disseny de la interfície d'usuari.

\subsection{Auditoria i Selecció de l'Arquitectura HPE}
Per garantir la màxima precisió biomecànica, s'ha realitzat una prova de concepte empírica comparant tres arquitectures sobre un vídeo real de gimnàstica en barra d'equilibris. Mitjançant l'aplicació \textit{CVAT}, s'ha generat un \textit{Ground Truth} manual per obtenir mètriques extrapolables al domini del projecte. Els resultats de la Taula \ref{tab:avaluacio-gym} justifiquen l'elecció de \textit{Sapiens-2B} per la seva superioritat davant d'oclusions severes.

\begin{table}[H]
\centering
\caption{Avaluació de models sobre fragment acrobàtic real (Ground Truth)}
...
\begin{figure}[H]
    ...
    \caption{Exemple d'efecte no desitjat (\textit{oversmoothing}) en l'extracció de coordenades durant un gir de perfil. El filtre de Savitzky-Golay difumina l'intercanvi de cames provocant una pèrdua de la morfologia real.}
    \label{fig:compare_bad}
\end{figure}

No obstant això, aquesta decisió es va revertir en comprovar la seva importància crítica per a l'estabilitat global del sistema. Durant les extraccions massives per al \textit{dataset} d'entrenament de la \textit{RNN} i els tests de l'\textit{E-Score}, s'ha demostrat que l'aplicació del filtre és fonamental per retenir fotogrames on la confiança de la xarxa davalla (\textit{frame dropping}). En acrobàcies d'extensió fluida, el filtre assoleix un alineament biomecànic quasi perfecte.
...
Aquesta dualitat de resultats ha permès concloure que els beneficis en l'estabilitat temporal superen els riscos puntuals de deformació. Per tant, s'ha optat per procedir amb l'algorisme per garantir un senyal espacial continu per al motor de regles.

\subsection{Creació del Dataset i Data Augmentation}
L'entrenament d'un classificador basat en xarxes recurrents exigeix un volum de dades representatiu i equilibrat. Per a la construcció del \textit{dataset} d'aquest projecte, s'han seleccionat majoritàriament vídeos procedents del canal oficial de \textit{USA Gymnastics}\cite{usa_gymnastics_yt}, identificant i retallant fragments de rutines de barra d'equilibris (\textit{Balance Beam}). L'objectiu ha estat aïllar la fase aèria de les quatre categories acrobàtiques definides (\textit{Pike, Split, Straddle} i \textit{Tuck}), obtenint una mostra inicial de 72 seqüències.
...
Aquesta operació ensenya a la xarxa a reconèixer la simetria biomecànica de l'exercici independentment de l'orientació del salt, i ha permès duplicar exactament el volum de dades per a cada categoria. El \textit{dataset} final ha quedat constituït per un total de 144 mostres estructurades.

A la Taula \ref{tab:dataset_stats} es detalla la distribució final de les dades recopilades. Com es pot observar, l'increment assolit amb el \textit{Data Augmentation} garanteix un \textit{dataset} amb un equilibri òptim entre categories, proporcionant una base de coneixement suficient i lliure de biaixos per a l'aprenentatge de les capes \textit{LSTM}.
...
\begin{tabular}{lcc}
\hline
\textbf{Categoria} & \textbf{Mostres Originals} & \textbf{Total Dades (+ Data Augm.)} \\ \hline
\textbf{Pike}     & 18 & 36 \\
\textbf{Split}    & 20 & 40 \\
\textbf{Straddle} & 17 & 34 \\
\textbf{Tuck}     & 17 & 34 \\ \hline
\textbf{TOTAL}    & \textbf{72} & \textbf{144} \\ \hline
\end{tabular}
\end{table}

\subsection{Motor de Lògica d'Avaluació i Pelvis Virtual}
Una vegada la xarxa \textit{LSTM} ha classificat l'acrobàcia, el sistema transfereix el control al mòdul \textit{AcrobaticEvaluator}. Aquest component actua com un motor determinista que tradueix les normatives del Codi de Puntuació de la \textit{FIG} \cite{fig_rules} en regles geomètriques computables. La responsabilitat d'aquest motor és identificar el \textit{peak frame} (fotograma de màxima amplitud) i aplicar-hi els llindars de penalització definits al mòdul \texttt{rules.py}.

Per garantir una avaluació robusta davant la distorsió de la perspectiva monocular, s'ha dissenyat la Pelvis Virtual. Es tracta d'un punt de control sintètic, que anomenarem \textbf{punt imaginari} ($P_i$), calculat com el punt mig geomètric entre els \textit{keypoints} dels malucs esquerre ($P_L$) i dret ($P_R$), corresponents als \textit{keypoints} 23 i 24 respectivament:
...
\textbf{Acrobàcia} & \textbf{Criteri} & \textbf{Minor} & \textbf{Medium} & \textbf{Severe} \\ 
 & & (-0.1) & (-0.3) & (-0.5) \\ \hline
\textbf{Tuck} & Maluc \newline Genoll & $< 65^\circ$ & $< 90^\circ$ & $< 100^\circ$ \\ \hline
\textbf{Pike} & Maluc & $< 65^\circ$ & $< 90^\circ$ & $< 100^\circ$ \\
 & Genoll & $> 170^\circ$ & $> 160^\circ$ & $> 145^\circ$ \\ \hline
\textbf{Split} & Obertura & $> 170^\circ$ & $> 160^\circ$ & $> 135^\circ$ \\
 & Genoll & $> 170^\circ$ & $> 160^\circ$ & $> 145^\circ$ \\ \hline
\textbf{Straddle} & Obertura & $> 170^\circ$ & $> 160^\circ$ & $> 135^\circ$ \\
 & Genoll & $> 170^\circ$ & $> 160^\circ$ & $> 145^\circ$ \\ \hline
\end{tabular}
\end{table}

Aquests llindars es vinculen directament al sistema de visualització mitjançant un mapeig de colors (\textit{Color Map}): les infraccions \textit{Minor} es representen en verd, les \textit{Medium} en taronja i les \textit{Severe} en vermell. El sistema calcula la desviació del fotograma pic respecte a la base de 10.0 punts, oferint un desglossament objectiu que el jutge pot validar mitjançant la futura interfície de \textit{PyQt6}.

\subsection{Avaluació Contínua i Mapeig Espacial}
Per tal de transcendir l'anàlisi de clips aïllats i permetre el processament de rutines completes, s'ha implementat un algorisme de finestra lliscant (\textit{sliding window}). Aquest sistema fragmenta el flux continu de vídeo en blocs temporals de $N=30$ fotogrames amb un desplaçament (\textit{stride}) constant. Cada bloc s'envia a la xarxa \textit{LSTM}, que retorna una probabilitat de pertinença a les classes acrobàtiques.

Un repte detectat en aquesta fase d'avaluació contínua és la detecció de "falsos positius" en segments de transició, especialment durant els aterratges (\textit{landings}). Atès que el \textit{dataset} d'entrenament inclou la fase final del salt per captar la biomecànica completa, el model pot classificar erròniament el contacte amb la barra com a part de l'acrobàcia. Aquesta limitació es pretén mitigar en la fase de poliment mitjançant la introducció d'una classe "Background/Transició". No obstant això, cal subratllar que el sistema s'ha dissenyat com una eina d'assistència al judici: el jutge humà manté el criteri final i pot descartar aquelles avaluacions automàtiques que corresponguin a fotogrames no rellevants biomecànicament.

D'altra banda, per visualitzar les penalitzacions de forma clara, s'ha resolt el repte del mapeig espacial de coordenades. Les dades emmagatzemades al JSON són valors normalitzats relatius al centre del tors (en un rang aproximat de $[-1, 1]$), un format optimitzat per a l'aprenentatge del model però no apte per a la representació gràfica directa. Per resoldre-ho, s'ha creat un llenç independent de $800 \times 800$ píxels i s'ha implementat una funció de denormalització lineal:

\begin{equation}
    X_{pixel} = (x_{norm} \cdot S) + O_x, \quad Y_{pixel} = (y_{norm} \cdot S) + O_y
\end{equation}

On $S$ és un factor d'escala constant per amplificar la magnitud de l'esquelet i $(O_x, O_y)$ representen l'òfset necessari per centrar la figura en el llenç. Aquesta transformació permet reconstruir l'esquelet virtual amb una proporció biomecànica perfecta, facilitant la il·luminació dinàmica dels \textit{joints} infractors (verd, taronja i vermell) segons la gravetat de la deducció calculada.

\subsection{Prototipatge de la Interfície Gràfica (GUI)}
Com a avançament estratègic de la Fase 4 del cronograma, dedicada al poliment i integració a l'aplicació d'escriptori per als jutges, s'ha iniciat el disseny de la interfície mitjançant el \textit{framework} \textit{PyQt6}. Cal subratllar que el mòdul es troba actualment en una fase de prototipatge visual no funcional, servint com a guia conceptual per validar la distribució d'elements abans de la seva implementació lògica final, així com per consolidar la familiarització amb les eines de desenvolupament seleccionades.

L'arquitectura prevista per a la interfície es basa en un sistema de doble visualització (\textit{Dual Viewport}):

\begin{itemize}
  \item \textbf{Panell de Referència i Control:} Està destinat a la visualització del flux de vídeo principal. Es preveu que l'usuari pugui alternar entre tres modalitats: el metratge original de la competició, l'output directe del model \textit{Sapiens} (esquelet en cru sobre el vídeo en format .avi) o la representació avançada que integra el codi de colors segons la gravetat de cada angle biomecànic, dades extretes directament de les metadades JSON.

  \item \textbf{Panell d'Anàlisi Biomecànic:} En aquest espai es renderitzarà l'esquelet virtual sobre un llenç normalitzat. Aquesta vista aïllada permet identificar les infraccions posturals de forma neta, utilitzant la il·luminació dinàmica de les articulacions per ressaltar les deduccions detectades pel motor de regles.
\end{itemize}

Així mateix, s'ha dissenyat una consola de traçabilitat asíncrona (vegeu la Figura \ref{fig:gui_mockup}) que, en versions futures, comunicarà els resultats del motor de lògica en temps real. El disseny actual prioritza la llegibilitat de les deduccions i l'exposició de la confiança del model, actuant com un full de ruta per a la integració definitiva de l'arquitectura.

Aquest enfocament reafirma la visió del projecte com una eina d'assistència: la interfície es concep perquè la intel·ligència artificial actuï com un observador que proposa correccions, deixant la potestat final de confirmar o descartar les penalitzacions al criteri expert del jutge humà.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{img/mockup.png}
    \caption{Disseny conceptual (\textit{mockup}) de la interfície gràfica. Aquesta versió preliminar serveix per definir l'arquitectura visual i la disposició dels registres d'avaluació de cara a la integració final a la Fase 4.}
